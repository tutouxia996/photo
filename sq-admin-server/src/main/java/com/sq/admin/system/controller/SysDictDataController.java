...
import com.sq.framework.web.vo.R;
...
public class SysDictDataController extends BaseController {
     ...
     * 查询字典数据详细
     */
    @PreAuthorize("@ss.hasPermi('system:dict:query,system:dict:view')")
    ...
    public AjaxResult add(@Validated @RequestBody SysDictData dict) {
        if (org.apache.commons.lang3.StringUtils.isBlank(dict.getDictValue())) {
            dict.setDictValue(IdWorker.getIdStr());
        }
        SysDictData repeatDict = dictDataService.selectDictDataByDict(dict);
        if (repeatDict != null) {
    ...
    public R getByType(String dictType) {
        List<SysDictData> data = dictTypeService.selectDictDataByType(dictType);
        return R.ok(data);
    }

    @GetMapping(value = "/selectOptions")
    public R getByType(String dictType, String dictValues) {
        List<SysDictData> data = dictDataService.selectDictDataByType(dictType, StringUtils.isBlank(dictValues) ? null : dictValues.split(","));
        return R.ok(data);
    }
}
