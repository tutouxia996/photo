        ...
        try {
            ...
            if (subIfd != null) {
                ...
                if (subIfd.containsTag(ExifSubIFDDirectory.TAG_FNUMBER)) {
                    info.setAperture("f/" + subIfd.getDescription(ExifSubIFDDirectory.TAG_FNUMBER));
                }
                if (subIfd.containsTag(ExifSubIFDDirectory.TAG_EXPOSURE_TIME)) {
                    info.setShutterSpeed(subIfd.getDescription(ExifSubIFDDirectory.TAG_EXPOSURE_TIME));
                }
                ...
                if (subIfd.containsTag(ExifSubIFDDirectory.TAG_FOCAL_LENGTH)) {
                    info.setFocalLength(subIfd.getDescription(ExifSubIFDDirectory.TAG_FOCAL_LENGTH));
                }
                if (subIfd.containsTag(ExifSubIFDDirectory.TAG_LENS_MODEL)) {
                    info.setLensInfo(subIfd.getDescription(ExifSubIFDDirectory.TAG_LENS_MODEL));
                }
            }
            ...
            if (ifd0 != null) {
                ...
                if (make != null || model != null) {
                    info.setCameraModel(((make == null ? "" : make) + " " + (model == null ? "" : model)).trim());
                }
